...
@DataJpaTest
@ActiveProfiles("test")
@Import({QuerydslConfig.class, CompanyOverviewService.class, CompanyInfoService.class, CompanySectorService.class})
class CompanyOverviewServiceTest {
	...
	void getOverview() {
		...
		assertThat(result.getMetrics().get(0).getLevel().name()).isEqualTo("GREEN");
	}
	...
	void getOverviewWithRiskAndNonRiskLatestVersions() {
		...
		assertThat(result.getMetrics())
			.extracting("key")
			.containsExactlyInAnyOrder("RISK_METRIC", "SAFE_METRIC");
		assertThat(result.getMetrics().stream()
